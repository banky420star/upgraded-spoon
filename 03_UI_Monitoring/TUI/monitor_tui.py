...
def main():
    ...
    if once:
        ...
        console.print("Press Ctrl+C to exit. For full supervision: run vps_agi_supervisor.ps1 as SYSTEM Task Scheduler task.\n")

    if once:
        # Safe single render for tool use / quick status
        try:
            from rich.console import Console
            snap_console = Console(force_terminal=True, width=140)
    ...
    try:
        with Live(build_dashboard(), refresh_per_second=0.15, screen=True, transient=True) as live:
            while True:
                time.sleep(7)
